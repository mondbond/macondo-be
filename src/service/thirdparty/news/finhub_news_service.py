...
def form_article_test(article_data):
  ...
  try:
    ...
    logger.info(
      f"Failed fetch article {url}. exception is {e} Return summary instead.")
    alternative_summary = article_data.get('headline',
                                           'No headline') + ". " + article_data.get(
...
# --HEADLINES
def fetch_company_news(ticker, filter_by_name=True, later_than_hours_filter=24,
    max_articles=5):
  counter = 0
  ticker = ticker.strip()
  now = datetime.utcnow()
  ...
  news_items = finnhub_client.company_news(ticker, _from=yesterday_str,
                                           to=today_str)
  ...
  for item in news_items:
    ...
    if filter_by_name:
      if (ticker not in item.get("headline", "") and ticker not in item.get(
          "summary", "")
          and ticker_to_company_name.get(ticker, '!@#!@#') not in item.get(
              "headline", "")
          and ticker_to_company_name.get(ticker, '23423') not in item.get(
              'summary', "")):
        logger.info(
          f"Skipping as no naming for {ticker}, headline: {item.get('headline', '')}.")
        continue
