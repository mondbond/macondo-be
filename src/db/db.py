...
# CLIP embedder for image embeddings (light AWS deployment version)
clip_embedder = OpenCLIPEmbeddings(model_name="ViT-B/32",
                                   checkpoint="laion2b_s34b_b79k")


# Abstract base class for financial report vector database
...
# In-memory implementation of the financial report vector database
class InMemoryFinReportVectorDBReport(FinReportVectorDB):
    def __init__(self):
        ...
        self.route_db.add_texts([prompt_manager.get_prompt("company_news_route")],
                                metadatas=[{
                                             "route": UserIntentionEnum.NEWS_ABOUT_COMPANY.name}],
                                ids=["INIT_0"])
        self.route_db.add_texts(
            [prompt_manager.get_prompt("company_price_fall_route")],
            metadatas=[{"route": UserIntentionEnum.ANALYSE_SHARE_PRISE.name}],
            ids=["INIT_1"])
        self.route_db.add_texts([prompt_manager.get_prompt("fin_report_route")],
                                metadatas=[{
                                             "route": UserIntentionEnum.COMPANY_INFORMATION_FROM_REPORT.name}],
                                ids=["INIT_2"])
        self.route_db.add_texts(
            [prompt_manager.get_prompt("other_financial_route")],
            metadatas=[{"route": UserIntentionEnum.OTHER_FINANCIAL_QUESTIONS.name}],
            ids=["INIT_3"])
    ...
    def get_rephrased_retriever(self, type=None, ticker=None):
        template = PromptTemplate(
            template=prompt_manager.get_prompt(
              'rephrase_question_for_similarity_search'),
            input_variables=["question"])
        base_retriever = self.report_db.as_retriever(search_type="similarity",
                                                     search_kwargs={"k": 5,
                                                                    "filter": {
                                                                      "ticker": ticker}})

        retriever = RePhraseQueryRetriever.from_llm(retriever=base_retriever,
                                                    llm=get_llm(), prompt=template)
    ...
    def add_new_report(self, documents, metadata):
        self.report_db.add_texts(documents,
                                 metadatas=[{"ticker": metadata["ticker"]} for _ in
                                            range(len(documents))],
                                 ids=[str(metadata["ticker"] + "_" + metadata[
                                   "date"] + str(i)) for i in range(len(documents))]
                                 )

    def store_image_itself(self, image_path: str, metadata: dict = None):
        db_id = self.image_db.add_images(
            uris=[image_path],
            metadatas=[metadata])
        return logger.info(f"Image saved with id = {db_id}")
