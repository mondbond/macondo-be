...
def chat_with_summary(query: str, session_id: int = 1):
    sys_template = SystemMessagePromptTemplate.from_template(
        "You are a helpful financial assistant. Your name is Obadon. You response to only users question and do not add any additional information. If you don't know the answer, just say that you don't know. Do not try to make up an answer.")
    ...
    chat_prompt = ChatPromptTemplate.from_messages(
        [sys_template, placeholder, user_template])
    ...
    return chat_runnable.invoke({"query": query},
                                config={"session_id": session_id})


def chat(query: str, session_id: int = 1):
    sys_emplate = SystemMessagePromptTemplate.from_template(
        "You are a helpful financial assistant. Your name is Obadon. You response to only users question and do not add any additional information. If you don't know the answer, just say that you don't know. Do not try to make up an answer.")
    placeholder = MessagesPlaceholder(variable_name="history")
    user_template = HumanMessagePromptTemplate.from_template("{query}")
    chat_prompt = ChatPromptTemplate.from_messages(
        [sys_emplate, placeholder, user_template])
    pipeline = chat_prompt | get_llm()
