...
def resolver_node(state: ReflectAnswerState) -> ReflectAnswerState|str:
  print(f"MARK RESOLVER NODE - Iteration {state['iteration']}")
...
def end_node(state: ReflectAnswerState) -> ReflectAnswerState:
  print(f"END NODE - Iteration {state['iteration']}. Reason: {state['finished_reason']}. First mark {state['marks'][0] if state['marks'] else 'N/A'}, last mark {state['marks'][-1] if state['marks'] else 'N/A'}")
